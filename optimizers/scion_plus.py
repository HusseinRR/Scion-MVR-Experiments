...
This implements SCION with momentum variance reduction (MVR) for
better performance on heavy-tailed noise scenarios.
"""
...
class SCIONPlus(SCION):
    """
    SCION++ optimizer with Momentum Variance Reduction (MVR).
    
    Extends SCION with momentum and variance reduction techniques
    to improve convergence on heavy-tailed noise.
    """
    ...
    def step(self, closure=None, grad_new_dict: dict = None, grad_old_dict: dict = None):
        """
        Performs a single MVR optimization step.
    ...
    def _mvr_step(self, grad_new_dict: dict, grad_old_dict: dict):
        """Internal MVR step implementation."""
        with torch.no_grad():
            for group in self.param_groups:
                lr = group['lr']
                scale = group['scale']
                ...
                for p in group['params']:
                    ...
                    if not unconstrained:
                        # For unit ball constraint, the LMO is -g_k/||g_k||
                        direction = -g_k / (torch.norm(g_k) + 1e-8)
